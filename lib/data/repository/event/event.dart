...
class EventRepo {
  ...
  Future<List<EventData>> getEvent() async {
    try {
      ...
      Response response = await dio.post("https://api-hp3ki.inovatiftujuh8.com/api/v1/event/get-event-saka",
        data: {
  ...
  Future<void> joinEvent({required int eventId}) async {
    try {
      ...
      await dio.post("${AppConstants.baseUrl}/content-service/event-join/add", 
        data: {
          "event_id": eventId,
          "user_id": ar.getUserId()
        }
